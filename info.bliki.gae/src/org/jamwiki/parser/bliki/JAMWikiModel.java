...
import org.jamwiki.parser.jflex.WikiHeadingTag; //import org.jamwiki.parser.jflex.WikiSignatureTag;
...
public class JAMWikiModel extends AbstractWikiModel {
  ...
  private static final WikiLogger logger = WikiLogger
      .getLogger(WikiHeadingTag.class.getName());
  ...
  public JAMWikiModel(ParserInput parserInput, ParserOutput document,
      String contextPath) {
   ...
   */
  // private static String buildTopicUrlNoEdit(String context, String
  // virtualWiki,
  ...
  public void addCategory(String categoryName, String sortKey) {
    fParserOutput.addCategory(getCategoryNamespace()
        + NamespaceHandler.NAMESPACE_SEPARATOR + categoryName, sortKey);
  }
  ...
  public void buildEditLinkUrl(int section) {
    if (fParserInput.getAllowSectionEdit()) {
      TagNode divTagNode = new TagNode("div");
      divTagNode.addAttribute("style",
          "font-size:90%;float:right;margin-left:5px;", false);
      ...
      try {
        ...
        url = LinkUtil.buildEditLinkUrl(fParserInput.getContext(), fParserInput
            .getVirtualWiki(), fParserInput.getTopicName(), null, section + 1);
      } catch (Exception e) {
        logger.severe("Failure while building link for topic "
            + fParserInput.getVirtualWiki() + " / "
            + fParserInput.getTopicName(), e);
      }
      TagNode aTagNode = new TagNode("a");
      aTagNode.addAttribute("href", url, false);
      aTagNode.addChild(new ContentToken(Utilities.formatMessage(
          "common.sectionedit", fParserInput.getLocale())));
