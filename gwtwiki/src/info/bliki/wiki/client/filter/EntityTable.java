 ...
 * @version $Revision: 1.3 $ ($Author: axelcl $)
 */
public final class EntityTable {
   ...
   * Entity map.
   */
  private Map entityNametable = new HashMap();

  /**
   * Entity map.
   */
  private Map entityShorttable = new HashMap();
   ...
   */
  private void install(Entity ent) {
    this.entityNametable.put(ent.getName(), ent);
    this.entityShorttable.put(new Short(ent.getCode()), ent);
  }
  ...
  public Entity lookup(String name) {
    return (Entity) this.entityNametable.get(name);
  }
  ...
  public String entityName(Short code) {
    ...
    String name = null;
    Entity ent = (Entity) this.entityShorttable.get(code);
    if (ent != null) {
      name = ent.getName();
    }
    return name;
